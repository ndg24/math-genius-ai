...
def _entities_to_choices(entities, answer):
    ...
    for i, entity in enumerate(entities):
        ...
        entity_dict[entity_name] = entity
        letter = chr(ord('a') + i)
        choices_template += '({letter}) {{{entity_name}}}'.format(letter=letter, entity_name=entity_name)
        if entity is answer:
...
def _kth_biggest_multichoice_question(context, entities, adjective, answer):
    entity_dict, choices_template, answer_choice = _entities_to_choices(entities, answer)
...
def sort(sample_args, count=None):
    ...
    entropy, sample_args = sample_args.peel()
    values = _unique_values(entropy, only_integers=random.choice([False, True]), count=count)
